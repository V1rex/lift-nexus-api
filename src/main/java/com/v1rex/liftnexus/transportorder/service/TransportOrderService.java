...
public class TransportOrderService {
    ...
    public TransportOrderResponse createTransportOrder(TransportOrderRequest request) {
        log.info("Creating TransportOrder for LoadUnit: {} from Bin: {} to Bin: {}",
            request.targetLoadUnitId(), request.sourceBinId(), request.destinationBinId());
        ...
        if (loadUnit.getCurrentBin() == null || !loadUnit.getCurrentBin().getId().equals(sourceBin.getId())) {
    ...
    @Transactional
    public TransportOrderResponse updateOrderStatus(Long id, TransportOrderStatusUpdateRequest request) {
    ...
    @Transactional(readOnly = true)
    public Page<TransportOrderResponse> searchOrders(TransportOrderStatus status, Integer minWeight, Pageable pageable) {
        return transportOrderRepository.searchOrders(status, minWeight, pageable)
    ...
    public TransportOrder findEntityById(Long id) {
        return transportOrderRepository.findById(id)
            .orElseThrow(() -> new ResourceNotFoundException("TransportOrder with ID " + id + " not found."));
    }

    private void checkStatusBeforeUpdate(Long id, TransportOrderStatus currentStatus, TransportOrderStatus newStatus) {
        if (currentStatus == TransportOrderStatus.COMPLETED) {
            throw new IllegalStateException("Cannot update TransportOrder " + id + " because it is already COMPLETED.");
        }
        if (currentStatus == TransportOrderStatus.IN_PROGRESS && newStatus == TransportOrderStatus.OPEN) {
            throw new IllegalStateException("Cannot roll back TransportOrder " + id + " from ACTIVE to OPEN.");
        }
        if (currentStatus == TransportOrderStatus.ASSIGNED && newStatus == TransportOrderStatus.OPEN) {
            throw new IllegalStateException("Cannot roll back TransportOrder " + id + " from ASSIGNED to OPEN.");
        }
    }
}