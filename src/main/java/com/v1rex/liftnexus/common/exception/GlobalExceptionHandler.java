...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiError> handleConstraintViolationException(
    ...
    return createErrorResponse(detailMessage, HttpStatus.BAD_REQUEST, request);
  }
  ...
  public ResponseEntity<ApiError> handleValidationException(
    ...
    return createErrorResponse(
        "Validation Failed: " + errorMessage, HttpStatus.BAD_REQUEST, request);
  }

  private ResponseEntity<ApiError> createErrorResponse(
      String message, HttpStatus status, HttpServletRequest request) {
    ApiError error =
        new ApiError(message, status.value(), LocalDateTime.now(), request.getRequestURI());
