 ...
 * <p>
 * Manages the full lifecycle of a dispatch-optimisation job: submitting new jobs,
 * monitoring their status, terminating running jobs, and persisting the final
 * forklift-to-transport-order assignments. Acts as the bridge between the
 * warehouse domain (bins, forklifts, transport orders) and the constraint-solving
 * engine.
 * <p>
 * <b>Job lifecycle:</b> {@code QUEUED → SOLVING → COMPLETED / FAILED / ABORTED}.
 */
@Service
@Slf4j
...
public class WarehouseDispatcherService {
   ...
   * <p>
   * Intended to be extended with reconciliation logic that detects when Timefold
   * has silently stopped solving (e.g., due to an internal error) while the job
   * is still marked as {@code SOLVING} in the database, and marks it as
   * {@code FAILED} accordingly.
   *
   ...
   * <p>
   * Loads all storage bins, forklifts, and pending transport orders from the
   * database into an in-memory {@link WarehouseSchedule} object. This serves as
   * the problem fact set for the Timefold constraint solver.
   * <p>
   * <b>Known issue:</b> Loading <i>all</i> entities without pagination is a
   * performance bottleneck for large datasets. A more selective fetching strategy
   * (cursor-based pagination, lazy fields, caching) should be implemented.
   *
   * @return A fully populated {@link WarehouseSchedule} representing the current
   ...
   * <p>
   * Creates a {@link DispatchJob} record in state {@code QUEUED}, then delegates
   * to {@link SolverManager#solveAndListen} with:
   * <ol>
   *   <li>A problem-fetcher that transitions the job to {@code SOLVING} and
   *       builds the current warehouse state.</li>
   *   <li>A best-solution consumer that persists the final assignments and
   *       marks the job as {@code COMPLETED} (or {@code FAILED}).</li>
   * </ol>
   *
   * @return The UUID assigned to the newly created optimisation job. The caller
   *         can use this ID to poll status ({@link #getJobStatusAndReconcile}) or
   *         to terminate the job ({@link #terminateOptimizationJob}).
   */
   ...
   * <p>
   * Only jobs in state {@code QUEUED} or {@code SOLVING} can be terminated.
   * Once terminated, the job is marked as {@code ABORTED} and any partial
   * results are discarded (the best-solution consumer checks for this flag).
   *
   * @param jobId The unique identifier of the job to terminate.
   * @throws DispatchJobInvalidStateException if the job is already in a
   *         terminal state ({@code COMPLETED}, {@code FAILED}, or {@code ABORTED}).
   */
  @Transactional
   ...
   * <p>
   * Transitions the job from {@code QUEUED} to {@code SOLVING} in the database
   * and then builds the current warehouse snapshot. If building the snapshot
   * fails, the job is marked as {@code FAILED} and the exception is rethrown
   * to Timefold.
   * <p>
   * <b>Note:</b> This method is annotated {@code @Transactional} so that the
   * status update and snapshot build happen within the same persistence context.
   *
  ...
  /**
   * Best-solution consumer callback invoked by Timefold when a solution is
   * available (either the final optimal solution or an intermediate best-effort
   * one).
   * <p>
   * Persists the solver's assignments back to the database:
   * <ul>
   *   <li>Updates forklift assignments on transport orders.</li>
   *   <li>Records assigned orders on forklifts.</li>
   * </ul>
   * <p>
   * If the job was {@code ABORTED} during solving, the solution is silently
   * discarded. Otherwise the job is marked as {@code COMPLETED} (or
   * {@code FAILED} if persistence fails). Even infeasible solutions are saved
   * as a best-effort fallback.
   *
   * @param solution The {@link WarehouseSchedule} produced by Timefold, which
   *                 contains the optimised assignments.
