...
public class LoadUnitService {
    ...
    public LoadUnitResponse createLoadUnit(LoadUnitRequest request) {
        ...
        if (loadUnitRepository.existsByTrackingCode(request.trackingCode())) {
            log.warn("Creation failed: Load unit with tracking code {} already exists", request.trackingCode());
        ...
        return loadUnitRepository
            ...
            .orElseThrow(
                () -> {
                    ...
                    return new ResourceNotFoundException("Load Unit with tracking code '" + trackingCode + "' not found.");
                }
            );
    }
