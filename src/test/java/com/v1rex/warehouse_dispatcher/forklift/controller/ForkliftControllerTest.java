package com.v1rex.warehouse_dispatcher.forklift.controller;
...
import com.v1rex.warehouse_dispatcher.forklift.dto.ForkliftLocationUpdateRequest;
...
import com.v1rex.warehouse_dispatcher.forklift.service.ForkliftService;
...
public class ForkliftControllerTest {
    ...
    class FindByIdTest {
        ...
        void shouldReturnForklift_WhenGetRequestIsMadeToFindById() throws Exception {
            ForkliftResponse mockDto = new ForkliftResponse(1L, 40, EquipmentType.STANDARD, new ArrayList<TaskResponse>(), null);

            when(forkliftService.findById(1L)).thenReturn(mockDto);

            mockMvc.perform(get("/api/v1/forklifts/1"))
                    .andDo(print())
        ...
        void shouldReturnNotFound_WhenGetRequestIsMadeToFindByIdWithNonExistingId() throws Exception {
            ...
            when(forkliftService.findById(nonExistingId))
                    .thenThrow(new ResourceNotFoundException("Forklift with " + nonExistingId + " not found."));

            mockMvc.perform(get("/api/v1/forklifts/{id}", nonExistingId)
    ...
    class FindAllForkliftsTest {
        ...
        void shouldReturnForklifts_WhenGetRequestIsMade() throws Exception {
            ForkliftResponse mockForkliftOne = new
                    ForkliftResponse(1L,
                            40,
            ...
            ForkliftResponse mockForkliftTwo = new
                    ForkliftResponse(2L,
                            50,
                                                            EquipmentType.STANDARD,
                                                            new ArrayList<>(),
            ...
            Page<ForkliftResponse> mockPage = new
                    PageImpl<>(List.of(mockForkliftOne, mockForkliftTwo));

            when(forkliftService.findAll(Mockito.any(Pageable.class)))
                    .thenReturn(mockPage);

            mockMvc.perform(get("/api/v1/forklifts")
                            .accept(MediaType.APPLICATION_JSON))
                    .andDo(print())
                    .andExpect(status().isOk())
    ...
    class SearchByTest {
        ...
        void shouldReturnFilteredPagedForklifts() throws Exception {
            ForkliftResponse heavyForklift = new
                    ForkliftResponse(3L,
                            2000,
                                                           EquipmentType.STANDARD,
                                                           new ArrayList<>(),
            ...
            when(forkliftService.findWithCapacityGreaterThan(Mockito.eq(1000), Mockito.any(Pageable.class)))
                    .thenReturn(mockPage);

            mockMvc.perform(get("/api/v1/forklifts/search")
    ...
    class CreateForkliftTest {
        ...
        void shouldCreateForkliftAndReturnCreated() throws Exception {
            ...
            ForkliftResponse generatedResponse = new ForkliftResponse(99L, 1500, EquipmentType.STANDARD, new ArrayList<>(), null);

            when(forkliftService.createForklift(Mockito.any(ForkliftRequest.class))).thenReturn(generatedResponse);

            mockMvc.perform(post("/api/v1/forklifts")
    ...
    class UpdateLocationTest {
        ...
        void shouldUpdateForkliftLocationAndReturnOk() throws Exception {
            ...
            ForkliftResponse updatedResponse = new ForkliftResponse(1L, 40, EquipmentType.STANDARD, new ArrayList<>(), null);

            when(forkliftService.updateForkliftLocation(Mockito.eq(1L), Mockito.eq(5L))).thenReturn(updatedResponse);

            mockMvc.perform(put("/api/v1/forklifts/1/location")
                            .contentType(MediaType.APPLICATION_JSON)
