...
public class TaskServiceTest {
    ...
    class CreateTask {
        ...
        void createTask_ShouldReturnResponse_WhenRequestIsValid(){
            when(locationService.findEntityById(pickLocationId))
                    .thenReturn(pickLocation);
            when(locationService.findEntityById(deliveryLocationId))
                    .thenReturn(deliveryLocation);
            ...
            LocationResponse dummyPick = new
                    LocationResponse(pickLocationId,
                                    10.0F,
                                    20.0F);

            LocationResponse dummyDelivery = new
                    LocationResponse(deliveryLocationId,
                                    20.0F,
                                    30.0F);

            TaskResponse mockResponse = new TaskResponse(100L,
                    dummyPick,
                    dummyDelivery,
                    750,
                    EquipmentType.STANDARD,
    ...
    class UpdateTask {
        ...
        void updateTask_ShouldReturnResponse_WhenTransitionIsValid() {
            ...
            TaskStatusUpdateRequest assignedRequest =
                    new TaskStatusUpdateRequest(TaskStatus.ASSIGNED);

            LocationResponse dummyPick = new
                    LocationResponse(1L,
                                    10.0F,
                                    20.0F);
            LocationResponse dummyDeliv = new LocationResponse(2L,
                    12.0F,
                    22.0F);

            TaskResponse mockResponse = new TaskResponse(taskId,
                    dummyPick,
                    dummyDeliv,
                    500,
                    EquipmentType.STANDARD,
        ...
        @Test
        @DisplayName("Should throw IllegalStateException when un-assigning a COMPLETED task to anything else")
    ...
    class SearchTasks{
        ...
        void searchTasks_ShouldReturnPage_WhenCriteriaIsValid() {
            ...
            Page<Task> mockTaskPage = new org.springframework.data.domain.PageImpl<>(java.util.List.of(task));
