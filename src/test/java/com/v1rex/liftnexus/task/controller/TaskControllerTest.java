...
public class TaskControllerTest {
  ...
  class GetTaskById {
    ...
    void shouldReturnTask_WhenGetRequestIsMadeToFindById() throws Exception {
      TaskResponse mockDto =
          new TaskResponse(10L, null, null, 15, EquipmentType.STANDARD, TaskStatus.OPEN, 5L);
      when(taskService.findById(10L)).thenReturn(mockDto);

      mockMvc
          .perform(get("/api/v1/tasks/10"))
          .andDo(print())
    ...
    void shouldReturnNotFound_WhenGetRequestIsMadeToFindByIdWithNonExistingId() throws Exception {
      ...
      mockMvc
          .perform(get("/api/v1/tasks/{id}", nonExistingId).accept(MediaType.APPLICATION_JSON))
          .andDo(print())
  ...
  class SearchTasks {
    @Test
    @DisplayName(
        "Should return 200 OK with a paginated list of tasks when search parameters are valid")
    void shouldReturnPagedTasks_WhenSearchParamsAreValid() throws Exception {
      TaskResponse responseDto =
          new TaskResponse(10L, null, null, 500, EquipmentType.STANDARD, TaskStatus.OPEN, null);
      Page<TaskResponse> mockPage = new PageImpl<>(java.util.List.of(responseDto));

      when(taskService.searchTasks(
              eq(TaskStatus.OPEN), eq(500), any(org.springframework.data.domain.Pageable.class)))
          .thenReturn(mockPage);
  ...
  class CreateTask {
    ...
    void shouldReturnCreatedTaskWithLocationHeader_WhenRequestBodyIsValid() throws Exception {
      ...
      TaskResponse mockResponse =
          new TaskResponse(42L, null, null, 750, EquipmentType.STANDARD, TaskStatus.OPEN, null);
  ...
  class UpdateTask {
    ...
    void shouldReturnUpdatedTask_WhenUpdatePayloadIsSuccessful() throws Exception {
      ...
      TaskResponse mockResponse =
          new TaskResponse(10L, null, null, 500, EquipmentType.STANDARD, TaskStatus.COMPLETED, 5L);
