...
class LoadUnitControllerTest {
    ...
    class CreateLoadUnitEndpoint {
        ...
        @Test
        @DisplayName("Should accept correct JSON configurations and return 201 Created status with accurate context locations")
        void shouldCreateLoadUnitAndReturnCreated() throws Exception {
            LoadUnitRequest validRequest = new LoadUnitRequest(
                    "LU-CTRL-01",
                    620,
                    LoadUnitStatus.STORED,
                    5L);
            LoadUnitResponse mockResponse = new
                    LoadUnitResponse(
                    42L,
                    "LU-CTRL-01",
                    620,
            ...
            mockMvc.perform(post("/api/v1/load-units")
        ...
        @Test
        @DisplayName("Should capture validation payload defects and decline request processing early with 400 Bad Request status")
        void shouldReturnBadRequestOnValidationFailure() throws Exception {
            ...
            LoadUnitRequest structuralDefectPayload = new
                    LoadUnitRequest(""
                    , -50,
                    LoadUnitStatus.EXPECTED,
                    null);
    ...
    class GetLoadUnitEndpoint {
        ...
        @Test
        @DisplayName("Should serialize response accurately with 200 OK status if database element match is confirmed")
        void shouldReturnLoadUnitWhenFound() throws Exception {
            LoadUnitResponse activeResponse = new
                    LoadUnitResponse(12L,
                    "LU-FOUND",
                    15,
                    LoadUnitStatus.SHIPPED,
                    null,
                    0L);
            when(loadUnitService.findById(12L)).thenReturn(activeResponse);

            mockMvc.perform(get("/api/v1/load-units/12"))
        ...
        @Test
        @DisplayName("Should transform infrastructure exception maps to standard 404 Not Found returns securely")
        void shouldReturnNotFoundOnMissingElement() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/load-units/404"))
    ...
    class FilterLoadUnitsEndpoint {
        ...
        @Test
        @DisplayName("Should accept requests and pass correct sorting parameter schemas out to consumer loops")
        void shouldReturnPaginatedListFilteredByStatus() throws Exception {
            LoadUnitResponse response = new
                    LoadUnitResponse(1L,
                    "LU-PAGED",
                    80,
                    LoadUnitStatus.STAGED,
                    2L,
                    0L);
            ...
            PageImpl<LoadUnitResponse> responsePage = new PageImpl<>(List.of(response), expectedPageable, 1);


            when(loadUnitService.findByStatus(LoadUnitStatus.STAGED, expectedPageable)).thenReturn(responsePage);

            mockMvc.perform(get("/api/v1/load-units/status/STAGED")
                            .param("page", "0")
                            .param("size", "20")) // Spring automagically appends sort="id,asc" based on @PageableDefault
                    .andExpect(status().isOk())
